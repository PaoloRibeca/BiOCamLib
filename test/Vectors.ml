...
let () =
  ...
  let module FV = FrequenciesVector(MakeComparableScalar(Float)) in
  ...
  let module FV = FrequenciesVector(MakeRComparableScalar(Float)) in
  let print_fvf fv = Printf.printf "["; FV.iter (Printf.printf " %g*%d") fv; Printf.printf " ]\n%!" in
  let a = FV.of_floatarray ~non_negative:true init in
  assert (FV.is_non_negative a);
  print_fvf a;
  ...
  FV.threshold_accum_abs 0.5 a |> print_fvf;
  FV.to_floatarray a |> print_fa;
  let module IV = FrequenciesVector(MakeComparableScalar(Int)) in
  let print_iv iv = Printf.printf "["; IV.iter (Printf.printf " %d*%d") iv; Printf.printf " ]\n%!" in
  let a = IV.of_floatarray ~non_negative:true init in
