(*
    ...
    Annotations_Formats.ml holds every per-format reader and writer of
    the annotation subsystem, together with the scaffolding they share:
    the [Hierarchy] S-expression parser, the [GenBankLocation] AST and
    its LOCATION-string entry points, the field codecs that turn a
    column into a strand, a phase, a score or an interval, and the
    [Writer_t] / [Format_t] interfaces the format modules satisfy.
    [GFF3], [GTF], [GenBank] and [Tabular] round-trip; [Tbl] writes
    only.  [Format] and [Writer] are the runtime-dispatch companions.

    It sits between [Annotations_Base], whose AST it builds, and
    [Annotations], which re-exports all of the above and adds the
    binary I/O, the validation actions, the extraction primitives and
    the selection register.  The split keeps [Annotations.ml] to the
    public surface: the format modules are two thirds of the code and
    reach into the base and the parsers only.
