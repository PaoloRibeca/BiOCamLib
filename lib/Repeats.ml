(*
    Repeats.ml -- (c) 2023-2026 Paolo Ribeca, <paolo.ribeca@gmail.com>

    This file is part of BiOCamLib, the OCaml foundations upon which
    a number of the bioinformatics tools I developed are built.

    Repeats.ml implements tools to locate repeats within sequences.
  ...
  struct
    ...
    let iter ?(maximum_repeat_length = max_int) ?(minimum_locus_length = 0) ?(verbose = false) f s =
      ...
      while !k < maximum_repeat_length && PrefixMultimap.cardinal !old > 0 do
        ...
        let red_k = !k - 1 and max_i = l - !k
        and current = ref PrefixMultimap.empty and count = ref 0 in
